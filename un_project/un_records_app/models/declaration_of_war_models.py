...
class DeclarationOfWar(models.Model):
    ...
    def clean(self):
        ...
        if (self.resolved) and not self.peace_date:
            raise ValidationError("Invalidation date must be set if the war is resolved.")
        
        # Ensure that invalidation_date is None if neither resolved is True
        if not (self.resolved) and self.peace_date:
            raise ValidationError("Invalidation date cannot be set unless the war is resolved.")
